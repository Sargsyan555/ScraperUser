...
export class ScraperCamspartService {
  private readonly sitemapUrl = 'https://spb.camsparts.ru/sitemap.xml';
  private readonly logger = new Logger(ScraperCamspartService.name);
  ...
  // ✅ Made public and now returns a string (file path) or null
  public async scrapeAndExport(): Promise<string | null> {
    ...
    return filePath; // ✅ Now returns the path
  }
  ...
  private async getProductData(url: string): Promise<ProductData | null> {
    try {
      ...
      this.logger.warn(
        `Failed to scrape product data from: ${url}`,
        error.message || error,
      );
      return null;
    }
